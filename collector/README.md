...
3. Edit `collector\\config.json`:
   ...
   - Set `api_key` to match **`ACTIVITY_API_KEY`** in the API `.env`, **or** use **`"from-dotenv"`** (or leave **`""`**) to read the key from **`.env`**: repo root when running from source, or the same folder as **`ActivityTrackerCollector.exe`** in the portable Windows zip.
